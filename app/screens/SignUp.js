    ...
    console.log('payload', payload)
     postData(APP_URL.login,payload).then(res=>{
      ...
      if(res.error){
        ...
        props.navigation.navigate('Verification',{PhoneNumber:PhoneNumber,phoneCode:phoneCode})
      }
     }).catch(err=>{
      Toast.show(
        {
  ...
  return (
    <SafeView customStyle={{backgroundColor: 'white'}}>
      <ScrollView
        ...
        />
        <SecondaryInput text={AppStrings.cryptoName} keyboardType={'default'} onChangeText={(val)=>setcrptoName(val)}/>
        <PhoneNumTextInput
          countryFlag={flag}
          customStyles={{borderColor:AppColors.blackExtraLight}}
          customLabelStyle={{color:AppColors.blackLight}}
          onChangeText={(val)=>setPhoneNumber(val)}
          phoneCode={phoneCode}
          onPressCode={()=>setshowPicker(true)}
        />
        <SecondaryInput
          text={AppStrings.enterSafety}
          editable={false}
          value={secretKey}
...
const styles = StyleSheet.create({
  ...
  imageContainer: {
    ...
    borderRadius: (WINDOW_HEIGHT * 0.12) / 2,
